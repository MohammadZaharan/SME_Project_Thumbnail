...
static int set_pipeline(string video_path, string thumbnail_path)
{
    ...
    if (dir1 == NULL)
    {
       ...
       return 1;
    			...
    			gst_bin_add_many(GST_BIN(pipeline), src, decode, convert,scale, rate,  capsfilter, encode, sink, NULL);
    			...
    			GstCaps *caps = gst_caps_new_simple("video/x-raw","framerate", GST_TYPE_FRACTION, 1, 1000,"width", G_TYPE_INT, 900,"height", G_TYPE_INT, 500,NULL);
    			...
    			if (!gst_element_link(src, decode) || !gst_element_link_many(convert,  scale,rate, capsfilter, encode, sink, NULL))
    			{
...
void usbset()
{
    const gchar *path = "/media/ee213071/";
    GDir *dir;
    ...
    if (dir == NULL)
    {
        g_printerr("UNABLE TO OPEN Directory\n"); // Print "no usb" instead of "no directory"
        return ;
    }
...
int internal_storage_set()
{
	//set the location of video and dest
    string video_path = "/home/ee213071/Downloads/playback/video/";
    string thumbnail_path = "/home/ee213071/Downloads/playback/thumbnail/";
    set_pipeline(video_path,thumbnail_path);
return 0;
}
